console.log('ANON KEY:', import.meta.env.VITE_SUPABASE_ANON?.slice(0, 30))
...
export async function fetchDivision(divisionId) {
  ...
  if (!data || data.length === 0) throw new Error(`No cache found for ${divisionId} — run the edge function first`)
  return { pools: data[0].data, fetchedAt: new Date(data[0].fetched_at) }
}
